      ...
      <div
        ...
        <div class="section-year">
          <p
            ...
            class="year">
            {{ getYear(timelineContent) }} - {{ getToYear(timelineContent) }}
          </p>
        </div>
        <TimelineItem 
...
export default {
  ...
  methods: {
    ...
    getYear(date) {
      ...
      const month = date.from.toLocaleDateString(locale, { month: 'long' })
      const year = date.from.getFullYear()
      return month + " " + year;
    },
    getToYear(date) {
      const locale = this.dateLocale || navigator.language
      const month = date.to.toLocaleDateString(locale, { month: 'long' })
